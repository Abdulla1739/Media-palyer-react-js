...
function Categories({ setremoveCategoryVideo }) {
  ...
  const videoDropped = async (e, categoryId) => {
    ...
    try {
      ...
      console.log(data);
      let selectedCategory = allCategoryName?.find(
      ...
      setremoveCategoryVideo(result);
    } catch (error) {
      console.log(error);
    }
  };
  const videoDragStared = (e,videoDetails,categoryId)=>{
  ...
  return (
    <>
      ...
      {allCategoryName && allCategoryName.length > 0 ? (
              ...
              <Row className="mt-2">
                {item.allVideos?.length > 0 &&
                  item.allVideos?.map((video) => (
                    <Col draggable={true} onDragStart={e=>videoDragStared(e,video,item.id)} key={video?.id} className="mb-4" sm={12} md={6} lg={4}>
